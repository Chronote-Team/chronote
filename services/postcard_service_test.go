...
func TestValidatePostcardTitle(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := validatePostcardTitle(tt.input)
			if tt.wantError != "" {
...
func TestValidatePostcardContent(t *testing.T) {
	tests := []struct {
		...
		{name: "too large", input: json.RawMessage(`{"text":"` + strings.Repeat("a", maxPostcardContentBytes) + `"}`), wantError: "content 长度不能超过 65536 字节"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := validatePostcardContent(tt.input)
			if tt.wantError != "" {
				if err == nil || err.Error() != tt.wantError {
					t.Fatalf("expected error %q, got %v", tt.wantError, err)
				}
...
func TestPostcardHelperNormalization(t *testing.T) {
	page, pageSize := normalizePagination(0, 100)
	if page != 1 || pageSize != 50 {
		t.Fatalf("expected normalized pagination 1/50, got %d/%d", page, pageSize)
	}

	sortBy, order := normalizeSort("UPDATED_AT", "invalid")
	if sortBy != "updated_at" || order != "desc" {
		t.Fatalf("expected updated_at/desc, got %s/%s", sortBy, order)
	}
	...
	if err := validateVisibility("friends"); err == nil || err.Error() != "visibility 无效" {
...
func TestCanAccessPostcard(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := canAccessPostcard(tt.userID, tt.authorID, tt.visibility); got != tt.want {
